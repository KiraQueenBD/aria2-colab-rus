  ...
  def startWebUi(self, token, dport, nServer, region, btc, configPath,
    ...
    try:
        ...
        if v:
          clear_output()
          loadingAn(name="lds")
          textAn("Токен Ngrok уже используется! Пробуем другой токен...", ty='twg')
        time.sleep(2)
        return True
    ...
    try:
        if not len(setting):
            if not checkAvailable(fullPath):
                if v:print(f"Файл недоступен: {fullPath}.")
                exx()
        ...
        if v:print(f"Ошибка доступа к файлу: {fullPath}.")
